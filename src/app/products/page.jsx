...
export default function ProductsPage() {
  ...
  const openEditModal = (product) => {
    setForm({
      ...product,
      brand: product.brand || "", // ✅ Load brand
      image: null,
  ...
  const handleAddOrEditProduct = async (e) => {
    ...
    formData.append("brand", form.brand); // ✅ Send brand
  ...
  if (isLoading) return <div className="ml-80 p-8 min-h-screen bg-gray-50">Loading...</div>;

  return (
    <div className="ml-80 p-8 min-h-screen bg-gray-50">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800">All Products</h1>
        <button
          onClick={openAddModal}
          className="flex items-center gap-2 px-4 py-2 bg-[#7C3AED] text-white rounded-full shadow hover:bg-[#6A2FCC]"
        >
        ...
        <table className="min-w-full">
          <thead className="bg-purple-100 text-gray-700 text-left text-sm">
            <tr>
              ...
              <th className="p-4">Brand</th> {/* ✅ Added brand column */}
            ...
            {products.length > 0 ? (
              products.map((product) => (
                <tr key={product._id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-4">{product.brand || "—"}</td> {/* ✅ Show brand */}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <form
            ...
            className="bg-white rounded-xl p-6 w-full max-w-3xl shadow-lg max-h-[90vh] overflow-y-auto"
          >
            ...
            {/* Variations Section */}
            <div className="col-span-2 border p-4 rounded mt-6">
              <h3 className="font-semibold mb-4">Variations</h3>
              {form.variations.map((variation, i) => (
                <div
                  key={i}
                  className="flex flex-wrap gap-3 mb-4 items-center border p-3 rounded"
                >
                  <input
                    type="text"
